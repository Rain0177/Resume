...
#resume-entry(
  ...
  *项目描述：*
  基于 C++实现的分布式即时通讯系统，支持 websocket、 http、 tcp 协议。接入层通过负载均衡接入客户端，逻辑层负责解析客户端请求，进行业务分发， 结合数据层、文件层和路由层协同工作，实现高效的消息与文件服务。本项目支持用户登录，单聊、群聊、未读消息计数，展示最近会话列表、在线与离线文件传输等服务，通过分布式部署可支撑海量在线用户。
